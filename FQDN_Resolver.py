...
class FQDN:
    ...
    def __init__(self, fqdn, class_0_1='?'):  # todo:我不会python的“反射” ； 没有处理“？”
        self.name = str(fqdn[2:-2])
        self.max_label_length = self.max_label_length_counter(fqdn)
        self.avg_label_length = self.avg_label_length_counter(fqdn)
        ...
        self.csv = [self.name, self.class_0_1, self.max_label_length, self.avg_label_length, self.labels_num,
